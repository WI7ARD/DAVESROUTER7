...
if all(importlib.util.find_spec(name) is not None for name in GPU_PACKAGES):
    ...
    candidates = [Path(_sys.prefix) / "Library" / "bin"]
    try:
        import site as _site

        _userbase = _site.getuserbase()
        if _userbase:
            candidates.append(Path(_userbase) / "Library" / "bin")
    except Exception:
        pass
    for runtime in candidates:
        if not runtime.is_dir() or runtime in runtime_dirs:
            continue
    ...
    print(f"[spec] bundling Intel GPU support: dpnp + dpctl + {runtime_dirs}")
else:
