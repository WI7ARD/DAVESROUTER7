...
class GPUBackend(ComputeBackend):
    ...
    def reset(self) -> None:
        """Forget a failed init so the next ``available`` retries (env changed)."""
        self._failed = False
        self._error = None
    ...
    def fits(self, cells: int, layers: int) -> tuple[bool, str]:
        ...
        if free is None:
            ...
            if self._xp is not None and self._detection.array_module == "dpnp":
                ...
                return ok, (
                    f"needs ~{need / 2**20:.0f} MiB over the {cap >> 20} MiB shared-memory "
                    "cap: falls back to CPU"
                )
            return False, "free GPU memory unknown: falls back to CPU"
