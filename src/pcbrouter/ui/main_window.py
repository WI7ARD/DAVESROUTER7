...
class MainWindow(QMainWindow):
    ...
    def open_board(self, path: Path) -> bool:
        # Parse itself runs on the GUI thread (heavy geometry follows in the
        # background via GeometryController): reject absurd files early and let
        # the wait cursor paint before blocking.
        try:
            if Path(path).stat().st_size > 512 * 1024 * 1024:
                dialogs.show_error(
                    self, "Could not open board", f"{path.name} is larger than 512 MiB."
                )
                return False
        except OSError:
            pass
