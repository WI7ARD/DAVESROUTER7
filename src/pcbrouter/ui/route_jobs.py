...
class WorkerHost:
    ...
    def submit(self, job_id: int, job: JobBase) -> None:
        # Never block the GUI thread on a full/dead worker queue.
        try:
            self.inbox.put((job_id, job), timeout=2.0)  # pickled by the feeder thread
        except queue.Full as exc:
            raise RuntimeError("routing worker is not accepting jobs") from exc
...
class InProcessHost:
    ...
    def submit(self, job_id: int, job: JobBase) -> None:
        ...
        try:
            self.inbox.put((job_id, job), timeout=2.0)
        except queue.Full as exc:
            raise RuntimeError("routing worker is not accepting jobs") from exc
    ...
    def kill(self) -> None:
        ...
        if self._active_job:
            self.cancel_job.value = self._active_job
        elif self.cancel_job.value == 0:
            self.cancel_job.value = -1
        self._dead = True
        self.inbox.put(None)
...
class RouteJobController(QObject):
    ...
    def submit(self, job: JobBase, on_done: Callable[[JobDone], None]) -> int | None:
        ...
        except Exception as exc:  # worker died between alive() and put()
            log.warning("[route:%d] submit failed: %r", job_id, exc)
            self.active = None
