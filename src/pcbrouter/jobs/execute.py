...
def _worker_gpu(reset: bool = False) -> Any:
    ...
    with _GPU_LOCK:
        ...
        if _GPU is None:
            ...
            reset_fn = getattr(_GPU, "reset", None)
            if reset_fn is not None and getattr(_GPU, "_failed", False):
...
def _fake(job: FakeJob, ctx: JobContext) -> Any:
    ...
    if job.outcome == "hang":
        while True:  # synthetic hang: still honour cancellation so tests/GUI can stop it
            ctx.check_cancel()
            time.sleep(0.05)
