...
def test_timeout_and_unresponsive_worker_are_stopped(
    ...
    run_until(lambda: bool(done), 15)  # hang now honours cancel cooperatively
    assert done[0].status == JobStatus.CANCELED.value
...
def test_every_gpu_setup_button_responds(
    ...
    # Install via a harmless command from the start: on no-GPU machines the
    # AUTO click answers "No GPU found" without starting anything; on machines
    # with a GPU it must never run a real multi-minute pip install in a test.
    monkeypatch.setattr(
        mod, "pip_command", lambda pkg: [sys.executable, "-c", f"print('pip {pkg} ok')"]
    )
    dlg.install_button.click()
    # No-GPU machines report "No GPU found"; machines with an Intel/NVIDIA GPU
    # run the mocked pip ("pip <pkg> ok"). Either way the button must answer,
    # and any mocked install must finish before the hand-picked one below.
    run_until(lambda: "No GPU found" in text() or "pip " in text(), 30)
    run_until(lambda: dlg.process is None, 30)
